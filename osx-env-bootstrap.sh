...
function bootstrap_brew() {
  ...
  brew install android-file-transfer android-studio caffeine docker drawio firefox gas-mask gimp github chromium hex-fiend iterm2 joplin karabiner-elements keepassxc macdown meld rar sequel-pro sqlitestudio tencent-lemon youdaodict visual-studio-code vlc vmware-fusion wireshark xmind xquartz
  #brew install v2rayu
  if [ $? -ne 0 ]; then
    return 1
  fi
